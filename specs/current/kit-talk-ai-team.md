...
A 90-minute, self-contained HTML slideshow at `_ask/docs/talks/method-and-tooling.html` that an AI team can open in a browser and present. The talk teaches the kit method and the `./ask` tooling. The facilitator demo gets one slide.
...
- Keyboard: next/previous (Arrow keys, Space, PageUp/PageDown). Home/End jump to first/last.
- Slide counter visible.
- Speaker notes exist per slide; toggle with `n`. Notes carry a suggested minute mark so a presenter can pace 90 minutes.
...
- File: `_ask/docs/talks/method-and-tooling.html`
...
- The HTML file opens locally and the full deck is reachable by keyboard.
