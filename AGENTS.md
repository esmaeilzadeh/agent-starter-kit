...
6. **Default path (guidance, not a lock):** stay on kit stages and **prepare each artifact** (do not skip documents). After one “defaults OK,” do not re-bless Plan/Review until **Accept**. If artifacts are missing and they are still on the path, prepare them. Off-path (“just code”) only when they explicitly leave — warn once and follow. See `_ask/policies/workflow.md`. Read `_ask/policies/` (`delegation.md`, `risk.md`, `verification.md`, `worktree.md`, `workflow.md`).
