...
- **Autocomplete:** both commands complete subcommands. After any subcommand, Tab lists that command’s options. Running `askit` (any command except `--complete`) installs completion into the user account: bash-completion drop-ins plus a one-time `~/.bashrc` / `~/.zshrc` hook. The user does not run `eval "$(./ask completion …)"`. A child process cannot enable Tab in the already-open shell; the next Tab works if bash-completion is loaded, otherwise the next new terminal.
...
- Autocomplete on both commands (bash and zsh). Install happens automatically on `askit`; no eval.
