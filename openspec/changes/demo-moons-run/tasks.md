...
- [ ] 2.3 Commit `train_output.json` so the tree is clean (`./ask check-clean` exits 0). Literal Path A (train then record-run) is expected to refuse here; that is a walkthrough finding, not a silent demo-doc edit.
- [ ] 2.4 `./ask record-run --run-id moons-narrow --commit-sha "$(git rev-parse HEAD)" --metric` equal to that file's `accuracy`; SHA equals HEAD
- [ ] 2.5 Commit record-run artifacts (`run_manifest.json`, `summary.json`, `results/RUN_REGISTRY.md`) so the next run starts clean

## 3. moons-wide

- [ ] 3.1 Commit `results/moons-wide/config.yaml` with `hidden_layer_sizes: [32]` on a clean tree after 2.5
- [ ] 3.2 Train `python3 scripts/train_moons.py --run-id moons-wide` so `results/moons-wide/train_output.json` contains `accuracy`
- [ ] 3.3 Commit `train_output.json` so the tree is clean
- [ ] 3.4 `./ask record-run` for `moons-wide` writes a registry row whose `git_sha` differs from `moons-narrow`
- [ ] 3.5 Commit `moons-wide` record-run artifacts and the registry update
