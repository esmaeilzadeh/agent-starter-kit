...
```text
_ask/skills/manifest.yaml  → authoritative human-reviewed pins
...
**Cloud:** project agents must run `./ask prepare` before roles that need Community Skills (do not rely on global installs). Optional documented escape: `--vendor` / `--copy` with committed `.agents/skills/` for environments that cannot prepare at session start — not the default.
...
Implementation: `_ask/skills/prepare-skills.sh`.
...
Kit Protocol Files (`.agents/ask/stages/*.md`, policies, templates, `_ask/guide/`, `_ask/spec/`) are **shipped by the kit**. `_ask/agents/*.md` are pointers. Community Skills are **prepared from pins**.
