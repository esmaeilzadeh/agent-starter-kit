...
- record exact commit SHA (`./ask record-result`);
...
```text
ask                                    # Agent Starter Kit dispatcher (only root command)
...
**Does not copy** `.agents/skills/` bodies; runs `./ask prepare` + `./ask sync` after overlay unless `--skip-prepare`.
...
```text
./ask upgrade --version <tag-or-sha>
```
