...
| **Yes — core** | Prepared Community Skills in `.agents/skills/` | Pinned external methods from `_ask/skills/manifest.yaml` |
...
- **`_ask/skills/manifest.yaml`** = Kit Protocol (pins, roles) — not a Cursor discovery path.
...
- Kit Protocol Files (`_ask/agents/*.md`) must **not** be copied into skill folders; they are workflow contracts, not Community Skills.
...
| --- | --- | --- |
| Clean worktree before work | `_ask/scripts/check-clean-worktree.sh` | Hook calls script on `beforeShellExecution` (e.g. matcher for `git commit`, `git checkout`, or kit `start-work.sh`) |
| Refuse dirty `start-work` | `_ask/scripts/start-work.sh` | Same — hook denies or routes to script; script remains canonical exit codes |
| Workstream preconditions | `_ask/scripts/check-workstream.sh` | `beforeShellExecution` matcher or `sessionStart` context injection listing active `work-id` |
| Verification / result SHA | `_ask/scripts/verify.sh`, `record-result.sh` | Optional `afterShellExecution` audit; not a substitute for scripts |
...
**Kit implication:** The kit’s **Engineering Agents** (`00`–`10`) are **Kit Protocol Files**, not a 1:1 `.cursor/agents/` tree. A thin binding should **not** clone eleven subagents (that would be a bespoke runtime). Optional: one readonly **verifier** or **research** subagent where Cursor isolation helps; stage contracts stay in `_ask/agents/`.
...
Commands are **steering** (prompt packaging). Prefer commands that say “read `_ask/agents/06-implement.md` and run `_ask/scripts/check-workstream.sh`” rather than duplicating protocol text.
...
| --- | --- | --- |
| Agent stage contracts | `_ask/agents/00-explore.md` … `10-accept.md` | Runtime-agnostic labor definitions |
| Policies | `_ask/policies/*.md` | Authority/delegation/risk — not Cursor-specific |
| Templates | `_ask/templates/*` | Artifact schemas |
| Skill pins | `_ask/skills/manifest.yaml` | Dependency lockfile, not skill bodies |
| Skill prep script | `_ask/skills/prepare-skills.sh` | Portable install entry |
| Deterministic guardrails | `_ask/scripts/check-clean-worktree.sh`, `start-work.sh`, `check-workstream.sh`, `verify.sh`, `record-result.sh` | Enforceable without Cursor |
...
1. **`AGENTS.md` ≤ ~30 lines** — operational checklist only (per kit spec §34); link to `_ask/` rather than inlining policies.
...
6. **Commands optional** — useful for human slash entry; stage truth stays in `_ask/agents/`.
...
Hook script invokes `_ask/scripts/check-clean-worktree.sh` or `_ask/scripts/check-workstream.sh` and maps exit codes to `{ "permission": "deny", ... }` per [Hooks — beforeShellExecution](https://cursor.com/docs/hooks).
