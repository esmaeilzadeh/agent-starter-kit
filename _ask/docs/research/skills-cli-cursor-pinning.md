...
**Kit context:** The Starter Kit defines `_ask/skills/manifest.yaml` (source + revision + role) and `_ask/skills/prepare-skills.sh` so agents prepare pinned Community Skills locally without vendoring skill trees into the product repo ([starter-kit spec §21](../../ai-agent-starter-kit-spec.md)).
...
_(The `skills/engineering/...` value in `skillPath` is from an upstream skills package layout inside the CLI lock — it is **not** the Starter Kit root `_ask/`.)_
...
```mermaid
flowchart LR
  M[_ask/skills/manifest.yaml] --> P[prepare-skills.sh]
...
1. Read `_ask/skills/manifest.yaml`.
...
| File | Owner | Contents |
| --- | --- | --- |
| `_ask/skills/manifest.yaml` | Kit | Human-reviewed intent: source, **revision pin**, role |
...
7. **No first-class manifest format in CLI** — `_ask/skills/manifest.yaml` is kit-specific; CLI has no `skills prepare --manifest` command.
