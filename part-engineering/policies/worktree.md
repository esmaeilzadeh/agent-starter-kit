...
1. Run `./pek check-clean` (or equivalent `git status --porcelain` check).
...
Every plan / workstream starts on its **own dedicated branch** (default: `agent/<work-id>` via `./pek start-work <work-id>`). Do not implement a plan on `main`/`master` or on another workstream’s branch.
